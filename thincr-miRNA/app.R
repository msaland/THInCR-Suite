...
my.env <- new.env()
autofill <- readRDS("../thincr-data/autofill-miRNA.rds")
pqvalues <- readRDS("pq-values-miRNA.rds")

ui <- fluidPage(title = "THInCR - miRNA vs. HPV Status", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("THInCR - Correlation of Cellular miRNA Expression and HPV Status"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to compare expression of candidate cellular miRNAs between HPV+, HPV- and normal control tissue from the TCGA cervical (CESC) and head & neck cancer (HNSC) cohorts. Select the candidate miRNA by typing the name in the input box on the top right. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Box plots can be downloaded as PNG files and the information necessary to reproduce the box plots can be downloaded as a CSV file. Level 3 miRNA expression data from the TCGA head & neck cancer (HNSC) and cervical carcinoma (CESC) cohorts was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". HPV status was annotated as described in Gameiro et al, Viruses 2017", a(href="https://www.mdpi.com/1999-4915/9/9/252", "DOI: 10.3390/v9090252")),
  ...
  sidebarLayout( sidebarPanel(width=6, selectizeInput(
    ...
    mainPanel(
      ...
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'hnsccdownload', 'Download HNSC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))), width = 6)),
...
server <- function(input, output, session) {
  ...
  output$cescdownload <- downloadHandler(
    ...
    content = function(filename){ggsave(filename, my.env$g.cesc.plot, width = 8, height = 6, dpi = 300)
    })
  
  output$hnsccdownload <- downloadHandler(
    filename = function() {paste("HNSC-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.hnscc.plot, width = 8, height = 6, dpi = 300)
    })
  ...
  output$tablePQ <- renderDT({
    ...
    assign("g.hnscc.miRNA", hnscc.miRNA, envir = my.env)
  ...
  output$tableStats <- renderDT({
    ...
    gene <- input$search
    assign("g.gene", gene, envir = my.env)
    cesc.miRNA <- my.env$g.cesc.miRNA
    hnscc.miRNA <- my.env$g.hnscc.miRNA
    cesc.hpv <- data.frame(cesc.miRNA[cesc.miRNA["HPV_Status"] == "HPV16" | cesc.miRNA["HPV_Status"] == "HPV33" | cesc.miRNA["HPV_Status"] == "HPV35", gene])
    ...
    cesc.neg <- data.frame(cesc.miRNA[cesc.miRNA$HPV_Status=="Negative", gene])
    ...
    cesc.norm <- data.frame(cesc.miRNA[cesc.miRNA$HPV_Status=="Normal", gene])
    ...
    hnscc.hpv <- data.frame(hnscc.miRNA[hnscc.miRNA["HPV_Status"] == "HPV16" | hnscc.miRNA["HPV_Status"] == "HPV33" | hnscc.miRNA["HPV_Status"] == "HPV35", gene])
    ...
    hnscc.neg <- data.frame(hnscc.miRNA[hnscc.miRNA$HPV_Status=="Negative", gene])
    ...
    hnscc.norm <- data.frame(hnscc.miRNA[hnscc.miRNA$HPV_Status=="Normal", gene])
  ...
  drawPlot <- function(plot.data, gene, titles) {
    ...
    my.plot <- ggplot(transform.data,aes(x=Status,ymin=One-1.5*(Median-One),lower=One,middle=Median,upper=Three,ymax=Three+1.5*(Three-Median),fill=Status))+ 
      ...
      ylab(paste("miRNA expression levels of", gsub("hsa-","",gene), sep=" ")) +
      ggtitle(titles) +
  ...
  output$boxPlotHNSCC <- renderPlot({
    ...
    hnscc.plot <- drawPlot(plot.data, gene, "HNSCC Dataset")
  ...
  makePlotData <- function(gene, mode) {
    if(mode == "CESC") {
      cesc.miRNA <- my.env$g.cesc.miRNA
      hpv <- data.frame(cesc.miRNA[cesc.miRNA["HPV_Status"] == "HPV16" | cesc.miRNA["HPV_Status"] == "HPV33" | cesc.miRNA["HPV_Status"] == "HPV35", gene])
      neg <- data.frame(cesc.miRNA[cesc.miRNA$HPV_Status=="Negative", gene])
      norm <- data.frame(cesc.miRNA[cesc.miRNA$HPV_Status=="Normal", gene])
    } else if (mode == "HNSC") {
      hnscc.miRNA <- my.env$g.hnscc.miRNA
      hpv <- data.frame(hnscc.miRNA[hnscc.miRNA["HPV_Status"] == "HPV16" | hnscc.miRNA["HPV_Status"] == "HPV33" | hnscc.miRNA["HPV_Status"] == "HPV35", gene])
      neg <- data.frame(hnscc.miRNA[hnscc.miRNA$HPV_Status=="Negative", gene])
      norm <- data.frame(hnscc.miRNA[hnscc.miRNA$HPV_Status=="Normal", gene])
    }
    hpv['Status'] = "HPV"
    names(hpv)[1] = "Value"
    neg['Status'] = "Negative"
