...
ui <- fluidPage(title = "THInCR - Probe Methylation vs. Genomic Loci", theme = shinytheme("cosmo"), useShinyjs(), 
  ...
  sidebarLayout(sidebarPanel( 
    ...
    mainPanel(width = 6,
      ...
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'CESCdownloadLine', 'Download CESC Lineplot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "linePlotHNSCC")),
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'HNSCCdownloadLine', 'Download HNSC Lineplot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "boxCESC")),
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'CESCdownloadBox', 'Download CESC Boxplot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "boxHNSCC")),
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'HNSCCdownloadBox', 'Download HNSC Boxplot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))))),
...
server <- function(input, output, session) {
  ...
  output$CESCdownloadLine <- downloadHandler(
    filename = function() {paste("CESC-lineplot-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.lineplotCESC, width = 15, height = 5, dpi = 300)
  })
  
  output$HNSCCdownloadLine <- downloadHandler(
    filename = function() {paste("HNSC-lineplot-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.lineplotHNSCC, width = 15, height = 5, dpi = 300)
  })
  
  output$CESCdownloadBox <- downloadHandler(
    filename = function() {paste("CESC-boxplot-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.boxplotCESC, width = 5, height = 5, dpi = 300)
    })
  
  output$HNSCCdownloadBox <- downloadHandler(
    filename = function() {paste("HNSC-boxplot-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.boxplotHNSCC, width = 5, height = 5, dpi = 300)
    })
  ...
  observeEvent(input$rangeBP,{
    ...
    if(nrow(my.env$g.part.data.cesc)<2 & nrow(my.env$g.part.data.hnscc)<2){
      hide("CESCdownloadLine")
      hide("HNSCCdownloadLine")
      hide("downloadData")
    } else{
      show("CESCdownloadLine")
      show("HNSCCdownloadLine")
      show("downloadData")
    }
  })
