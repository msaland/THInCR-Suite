...
my.env <- new.env()
autofill1 <- readRDS("../thincr-data/autofill-miRNA.rds")
...
ui <- fluidPage(title = "THInCR - miRNA vs. Immune Function", theme = shinytheme("cosmo"), useShinyjs(),
        ...
        navbarPage(title="Other Links:", tabPanel(title="☲"), navbarMenu(title="My Apps", tabPanel(a(href="https://thincr.ca/thincr-home/", "Home")), tabPanel(a(href="https://thincr.ca/thincr-mrna/", "Cellular mRNA Expression vs. HPV Status")), tabPanel(a(href="https://thincr.ca/thincr-mirna/", "Cellular miRNA Expression vs. HPV Status")), tabPanel(a(href="https://thincr.ca/thincr-correlation-mrna/", "Cellular mRNA vs. HPV mRNA Expression")), tabPanel(a(href="https://thincr.ca/thincr-correlation-mirna/", "Cellular mRNA vs. HPV miRNA Expression")), tabPanel(a(href="https://thincr.ca/thincr-imm-mrna/", "Cellular mRNA Expression vs. Immune Function")), tabPanel(a(href="https://thincr.ca/thincr-imm-mirna/", "Cellular miRNA Expression vs. Immune Function")),tabPanel(a(href="https://thincr.ca/thincr-survival-mrna/", "Cellular mRNA Expression vs. Overall Survival")),tabPanel(a(href="https://thincr.ca/thincr-survival-mirna/", "Cellular miRNA Expression vs. Overall Survival")), tabPanel(a(href="https://thincr.ca/thincr-methyl-all/", "Cellular loci vs. Methylation Status")))),
        titlePanel("THInCR - Correlation of Cellular miRNA Expression and Immune Function by HPV Status"),
        p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between candidate cellular miRNA expression and various calculated characteristics related to the tumor immune landscape in HPV positive cervical (CESC) and head & neck cancers (HNSC). Select the candidate cellular miRNAs by typing the name in the input box on the top right. Select the immune function/factor to analyze by selecting the desired option in the adjacent dropdown. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Correlation plots can be downloaded as PNG files and the information necessary to reproduce the box and scatter plots can be downloaded as a CSV file. Level 3 miRNA expression data from the TCGA head & neck cancer (HNSC) and cervical carcinoma (CESC) cohorts was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". HPV status was annotated as described in Gameiro et al, Viruses 2017", a(href="https://www.mdpi.com/1999-4915/9/9/252", "DOI: 10.3390/v9090252"), ". Immune landscape data was extracted from Thorsson et al, Immunity 2018", a(href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5982584/", "DOI: 10.1016/j.immuni.2018.03.023")),
        ...
        p(style="text-align: justify;", strong("Citing THInCR: "),"TBA (Website put together by Mikhail Salnikov under the supervision of Dr. Joseph Mymryk)"),
        sidebarLayout( sidebarPanel(width=6,
          fluidRow(column(7, selectizeInput(inputId = 'searchGene', label = HTML('Type the miRNA ID or Name'),
                                            choices = autofill1, selected = character(0), options = list(placeholder = 'Please type in the miRNA name/ID or select from the list', onInitialize = I('function() { this.setValue(""); }'), openOnFocus = FALSE), multiple = FALSE)),
          ...
          mainPanel(
            ...
            fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'cescdownloadScatter', 'Download CESC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
            jqui_resizable(plotOutput(outputId = "boxPlotCESC")),
            fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'cescdownloadBox', 'Download CESC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
            jqui_resizable(plotOutput(outputId = "scatterPlotHNSCC")),
            fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'hnsccdownloadScatter', 'Download HNSC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))), 
            jqui_resizable(plotOutput(outputId = "boxPlotHNSCC")),
            fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'hnsccdownloadBox', 'Download HNSC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))), width = 6)),
...
server <- function(input, output, session) {
  ...
  output$cescdownloadScatter <- downloadHandler(
    filename = function() {paste("CESC-scatter-", my.env$g.gene, "-", my.env$g.factor, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.cesc.plotScatter, width = 10, height = 10, dpi = 300)
    })
  
  output$cescdownloadBox <- downloadHandler(
    filename = function() {paste("CESC-box-", my.env$g.gene, "-", my.env$g.factor, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.cesc.plotBox, width = 10, height = 10, dpi = 300)
    })
  
  output$hnsccdownloadScatter <- downloadHandler(
    filename = function() {paste("HNSC-scatter-", my.env$g.gene, "-", my.env$g.factor, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.hnscc.plotScatter, width = 10, height = 10, dpi = 300)
    })
  
  output$hnsccdownloadBox <- downloadHandler(
    filename = function() {paste("HNSC-box-", my.env$g.gene, "-", my.env$g.factor, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.hnscc.plotBox, width = 10, height = 10, dpi = 300)
  })
  
  output$table <- renderDT({
    hide("cescdownloadScatter")
    hide("cescdownloadBox")
    hide("hnsccdownloadScatter")
    hide("hnsccdownloadBox")
    hide("downloadCSV")
    ...
    show("cescdownloadScatter")
    show("cescdownloadBox")
    show("hnsccdownloadScatter")
    show("hnsccdownloadBox")
    show("downloadCSV")
    show("downloadFactorCSV")
    gene <- input$searchGene
    factor <- gsub(" ","-",input$searchFactor)
    assign("g.factor", factor, envir = my.env)
    cesc.miRNA <- readRDS(paste("../thincr-data/mirna-genes/CESC-", gene, ".rds", sep=""))
    cesc.combined <- merge(cesc.miRNA, cesc.imm, by="TCGA-Call-Number")
    assign("g.cesc.combined", cesc.combined, envir = my.env)
    hnscc.miRNA <- readRDS(paste("../thincr-data/mirna-genes/HNSC-", gene, ".rds", sep=""))
    hnscc.combined <- merge(hnscc.miRNA, hnscc.imm, by="TCGA-Call-Number")
    ...
    hnscc.hpv.gene.summary <- c(paste("HNSCC HPV(+) - ", gene, sep=" "), as.numeric(count(hnscc.hpv.gene)$n), round(as.numeric(min(hnscc.hpv.gene)), digits = 3), round(as.numeric(max(hnscc.hpv.gene)), digits = 3), round(mean(unlist(hnscc.hpv.gene)), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    hnscc.neg.gene <- data.frame(hnscc.combined[hnscc.combined$HPV_Status.x=="Negative", gene])
    names(hnscc.neg.gene)[1] = "Value"
    hnscc.neg.gene.summary <- c(paste("HNSCC HPV(+) - ", gene, sep=" "), as.numeric(count(hnscc.neg.gene)$n), round(as.numeric(min(hnscc.neg.gene)), digits = 3), round(as.numeric(max(hnscc.neg.gene)), digits = 3), round(mean(unlist(hnscc.neg.gene)), digits = 3), round(as.numeric(summarize_all(hnscc.neg.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    ...
    hnscc.hpv.factor.summary <- c(paste("HNSCC HPV(+) -", gsub("-"," ",factor), sep=" "), as.numeric(count(hnscc.hpv.factor)$n), round(as.numeric(min(hnscc.hpv.factor)), digits = 3), round(as.numeric(max(hnscc.hpv.factor)), digits = 3), round(mean(unlist(hnscc.hpv.factor)), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.factor, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.factor, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv.factor, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    ...
    hnscc.neg.factor.summary <- c(paste("HNSCC HPV(-) -", gsub("-"," ",factor), sep=" "), as.numeric(count(hnscc.neg.factor)$n), round(as.numeric(min(hnscc.neg.factor)), digits = 3), round(as.numeric(max(hnscc.neg.factor)), digits = 3), round(mean(unlist(hnscc.neg.factor)), digits = 3), round(as.numeric(summarize_all(hnscc.neg.factor, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg.factor, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg.factor, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
  ...
  output$tableFactor <- renderDT({
    ...
    assign("g.gene", gene, envir = my.env)
    tablePR <- readRDS(paste("../thincr-data/imm-stats-mirna/", gene, ".rds", sep=""))
    pval.CP = c()
    pval.CN = c()
    pval.HP = c()
    pval.HN = c()
    for (i in 1:length(listFactor)){
      ...
      hnsc.plot.data <- makePlotDataScatter(gene, listFactor[i], "HNSC")
  ...
  drawPlotScatter <- function(plot.data, gene, factor, titles) {
    my.plot <- ggplot(data = plot.data, aes(x=ValueFactor, y=ValueGene, color=Status)) + 
      ...
      ylab(paste("miRNA expression levels of", gene, sep=" ")) +
      ggtitle(titles) +
  ...
  output$scatterPlotHNSCC <- renderPlot({
    ...
    gene <- input$searchGene
    factor <- gsub(" ","-",input$searchFactor)
    plot.data <- makePlotDataScatter(gene, factor, "HNSC")
  ...
  output$boxPlotHNSCC <- renderPlot({
    ...
    gene <- input$searchGene
    factor <- gsub(" ","-",input$searchFactor)
    plot.data <- makePlotDataBox(gene, factor, "HNSC")
    pos=unlist(unname(plot.data[plot.data[2]=="HPV",][1]))
    neg=unlist(unname(plot.data[plot.data[2]=="Negative",][1]))
    transform.pos=data.frame(signif(max(pos, na.rm=TRUE),3), signif(min(pos, na.rm=TRUE),3), signif(median(pos, na.rm=TRUE),3), quantile(pos, c(0.25,0.75), na.rm=TRUE)[1], quantile(pos, c(0.25,0.75), na.rm=TRUE)[2], "Positive")
    colnames(transform.pos)=c("Max", "Min", "Median", "One", "Three", "Status")
  ...
  output$textCESC1B <- renderUI({
    ...
    p.values <- compare_means(ValueFactor ~ Status,  data = plot.data)$p
