...
my.env <- new.env()
autofill <- readRDS("../thincr-data/autofill-mRNA.rds")
...
server <- function(input, output, session) {
  ...
  output$tableStats <- renderDT({
    ...
    hnscc.hpv.summary <- c("HNSCC - HPV(+)", as.numeric(count(hnscc.hpv)$n), round(as.numeric(min(hnscc.hpv)), digits = 3), round(as.numeric(max(hnscc.hpv)), digits = 3), round(mean(unlist(hnscc.hpv)), digits = 3), round(as.numeric(summarize_all(hnscc.hpv, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.hpv, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    hnscc.neg <- data.frame(hnscc.mRNA[hnscc.mRNA$HPV_Status=="Negative", gene])
    names(hnscc.neg)[1] = "Value"
    hnscc.neg.summary <- c("HNSCC - HPV(-)", as.numeric(count(hnscc.neg)$n), round(as.numeric(min(hnscc.neg)), digits = 3), round(as.numeric(max(hnscc.neg)), digits = 3), round(mean(unlist(hnscc.neg)), digits = 3), round(as.numeric(summarize_all(hnscc.neg, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.neg, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    hnscc.norm <- data.frame(hnscc.mRNA[hnscc.mRNA$HPV_Status=="Normal", gene])
    names(hnscc.norm)[1] = "Value"
    hnscc.norm.summary <- c("HNSCC - Normal", as.numeric(count(hnscc.norm)$n), round(as.numeric(min(hnscc.norm)), digits = 3), round(as.numeric(max(hnscc.norm)), digits = 3), round(mean(unlist(hnscc.norm)), digits = 3), round(as.numeric(summarize_all(hnscc.norm, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.norm, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(hnscc.norm, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
