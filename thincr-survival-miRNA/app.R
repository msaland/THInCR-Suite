...
ui <- fluidPage(title="THInCR - miRNA vs. OS", theme = shinytheme("cosmo"), useShinyjs(),
      ...
      titlePanel("THInCR - Correlation of Cellular miRNA Expression and Overal Survival by HPV Status"),
      p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between expression of a candidate miRNA and patient overall survival (OS) between HPV+ and HPV- cervical (CESC) and head & neck cancers (HNSC). Select the candidate miRNA by typing the name in the input box on the top right. Patients can be stratified into 2, 3 or 4 comparison groups based on expression levels of the target miRNA using the adjacent input box. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Kaplan-Meier plots can be downloaded as PNG files. Survival data was extracted from Liu et al, Cell 2018",a(href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6066282/", "DOI: 10.1016/j.cell.2018.02.052")),
      ...
      fluidRow(column(8, align="left", offset = 0, selectizeInput(
        inputId = 'search', label = HTML('Type the miRNA ID or Name'),
        choices = autofill, selected = character(0), options = list(placeholder = 'Please type in the miRNA name/ID or select from the list', onInitialize = I('function() { this.setValue(""); }'), openOnFocus = FALSE), multiple = FALSE)),
        column(4, align="left", offset = 0, selectizeInput(
           inputId = 'compare', label = HTML('Select # of Comparison Groups'),
    ...
    mainPanel(
      ...
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'cescdownload1', 'Download HPV(+) CESC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "survNegCESC", height=500)), 
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'cescdownload2', 'Download HPV(-) CESC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "survPosHNSCC", height=500)), 
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'hnsccdownload1', 'Download HPV(+) HNSCC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))),
      jqui_resizable(plotOutput(outputId = "survNegHNSCC", height=500)),
      fluidRow(column(6, align="center", offset = 3, downloadButton(outputId = 'hnsccdownload2', 'Download HPV(-) HNSCC Plot'), tags$style(type='text/css', "#button { vertical-align- middle; height- 50px; width- 100%; font-size- 30px;}"))), width = 6)),
...
server <- function(input, output, session) {
  ...
  output$cescdownload1 <- downloadHandler(
    filename = function() {paste("CESC-HPV-Pos-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.cesc.plot1[[1]], width = 10, height = 6, dpi = 300)
    })
  
  output$cescdownload2 <- downloadHandler(
    filename = function() {paste("CESC-HPV-Neg-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.cesc.plot2[[1]], width = 10, height = 6, dpi = 300)
    })
  
  output$hnsccdownload1 <- downloadHandler(
    filename = function() {paste("HNSCC-HPV-Pos-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.hnscc.plot1[[1]], width = 10, height = 6, dpi = 300)
    })
  
  output$hnsccdownload2 <- downloadHandler(
    filename = function() {paste("HNSCC-HPV-Neg-", my.env$g.gene, ".png", sep="")},
    content = function(filename){ggsave(filename, my.env$g.hnscc.plot2[[1]], width = 10, height = 6, dpi = 300)
    })
  ...
  makePlotData <- function(gene, mode) {
    if (mode == "CESC"){
      ...
      cesc.combined <- merge(cesc.miRNA, cesc.survival, by="TCGA-Call-Number")
      ...
      hnscc.combined <- merge(hnscc.miRNA, hnscc.survival, by="TCGA-Call-Number")
